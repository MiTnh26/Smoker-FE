...
import { 
  ...
  Search, Filter, Play, CheckCircle, X, ExternalLink
} from 'lucide-react';
...
const REASON_LABELS = {
  'ready_to_resume': 'Sẵn sàng tiếp tục',
  'content_updated': 'Đã cập nhật nội dung',
  'budget_added': 'Đã bổ sung ngân sách',
  'campaign_renewed': 'Gia hạn chiến dịch',
  'other': 'Khác'
};

export default function ManageResumeRequests() {
  ...
  const loadRequests = async () => {
    try {
      ...
      const response = await adminApi.getResumeRequests(params);
  ...
  const handleApprove = async (requestId) => {
    if (!window.confirm('Bạn đã resume banner trên Revive chưa? Sau khi duyệt, quảng cáo sẽ được tiếp tục.')) {
      return;
    }
    
    setProcessing(true);
    try {
      const response = await adminApi.approveResumeRequest(requestId, {
      ...
      console.error('Error approving resume request:', err);
  ...
  const handleReject = async (requestId) => {
    ...
    setProcessing(true);
    try {
      const response = await adminApi.rejectResumeRequest(requestId, {
        adminNote
      });
      ...
      console.error('Error rejecting resume request:', err);
      alert(err.response?.data?.message || 'Có lỗi xảy ra');
    } finally {
      setProcessing(false);
    }
  ...
  const handleComplete = async (requestId) => {
    setProcessing(true);
    try {
      const response = await adminApi.completeResumeRequest(requestId);
      ...
      console.error('Error completing resume request:', err);
      alert(err.response?.data?.message || 'Có lỗi xảy ra');
    } finally {
      setProcessing(false);
    }
    ...
    <div className={cn('w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6')}>
      ...
      <div className={cn('flex items-center justify-between mb-6')}>
        <div>
          <h1 className={cn('text-2xl md:text-3xl font-bold text-foreground mb-1')}>
            Yêu cầu tiếp tục quảng cáo
          </h1>
          <p className={cn('text-sm text-muted-foreground')}>
            Quản lý các yêu cầu tiếp tục quảng cáo từ BarPage
          </p>
        </div>
      </div>
      ...
      {filteredRequests.length === 0 ? (
        <div className={cn('text-center py-16 bg-card rounded-xl border border-border')}>
          <Play className={cn('w-12 h-12 mx-auto mb-3 text-muted-foreground/50')} />
            ...
            >
              <div className={cn('flex items-start justify-between')}>
                ...
                <div className={cn('flex flex-col gap-2 ml-4')}>
                  ...
                  {request.Status === 'approved' && !request.ReviveResumed && (
                    <button
                      onClick={() => handleComplete(request.ResumeRequestId)}
                      disabled={processing}
                      className={cn(
                        'px-4 py-2 rounded-lg text-sm font-medium',
        ...
        <div className={cn('fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4')}>
          <div className={cn('bg-background rounded-xl shadow-xl max-w-2xl w-full p-6 max-h-[90vh] overflow-y-auto')}>
            ...
            <div className={cn('flex gap-3 justify-end')}>
              ...
              <button
                onClick={() => handleReject(selectedRequest.ResumeRequestId)}
              ...
              <button
                onClick={() => handleApprove(selectedRequest.ResumeRequestId)}
                disabled={processing}
                className={cn(
                  'px-4 py-2 rounded-lg text-sm font-medium',
                  'bg-green-600 text-white hover:bg-green-700 transition-colors',
