...
export default function Reports() {
  ...
  const handleViewTargetDetail = async () => {
    if (!detail) return;
    const { TargetType, TargetId, originalTargetId, rawDescription, isLegacyPostReport, hasValidPostId } = detail;
    ...
    try {
      ...
      if (TargetType === "post" || TargetType === "Post") {
        ...
        let postId = originalTargetId || TargetId;
        
        // If postId is GUID format, try to get from rawDescription
        const isGuidFormat = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i.test(String(postId));
        if (isGuidFormat && rawDescription) {
          try {
            const desc = JSON.parse(rawDescription);
            if (desc.originalPostId) {
              postId = desc.originalPostId;
            }
          } catch {
            // rawDescription is not JSON, ignore
          }
        }
    ...
    <div className={cn("w-full max-w-7xl mx-auto px-4 md:px-6 lg:px-8 py-6")}>
      <div className={cn("flex items-center justify-between gap-3 mb-6 flex-wrap")}>
        ...
        <h1 className={cn("text-2xl md:text-3xl font-bold text-foreground")}>
            {t("admin.reports.title", { defaultValue: "Quản lý Report" })}
        </h1>
      </div>
        <button
          type="button"
      ...
      <div className={cn("rounded-lg bg-card border-[0.5px] border-border/20 overflow-hidden")}>
        ...
        {loading ? (
          ...
          <div className="py-10 text-center text-muted-foreground">Không có report</div>
        ) : (
          reports.map((r) => (
            <div key={r.ReportId || r.reportId} className="grid grid-cols-12 px-4 py-3 border-b border-border/10 text-sm items-center">
              ...
              <div className="col-span-2 flex justify-end gap-2">
                ...
                <button
                  ...
                  onClick={() => handleUpdateStatus(r.ReportId || r.reportId, "Resolved")}
                >
                ...
                <button
                  ...
                  className="px-3 py-1 rounded-md border text-xs hover:bg-muted/50"
                  disabled={updatingId === (r.ReportId || r.reportId)}
                  onClick={() => handleUpdateStatus(r.ReportId || r.reportId, "Ignored")}
                >
      ...
      {detail && (
        <div className="fixed inset-0 z-[1200] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" onClick={() => setDetail(null)}>
          <div
            className="bg-card rounded-lg border border-border/20 w-full max-w-2xl p-6 my-8 shadow-xl"
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="p-3 bg-muted/20 rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <div className="font-semibold text-foreground">Đối tượng bị báo cáo:</div>
                  <button
                    type="button"
                  ...
                  {detail.targetInfo && (
                    <>
                      ...
                      {detail.targetInfo.status && (
                        <div><span className="font-medium text-muted-foreground">Trạng thái:</span> <span className="text-foreground">{detail.targetInfo.status}</span></div>
                      )}
                    </>
                  )}
              ...
              {/* Reporter Info */}
              <div className="p-3 bg-muted/20 rounded-lg">
                <div className="font-semibold mb-2 text-foreground">Người báo cáo:</div>
                <div className="space-y-1.5">
                  {detail.reporterInfo && (
                    <>
                      <div><span className="font-medium text-muted-foreground">Tên:</span> <span className="text-foreground">{detail.reporterInfo.name}</span></div>
                      <div><span className="font-medium text-muted-foreground">Vai trò:</span> <span className="text-foreground">{detail.reporterInfo.role || detail.ReporterRole || detail.reporterRole}</span></div>
                    </>
                  )}
              ...
              {/* Reason & Description */}
              <div>
                <div className="font-semibold mb-2">Lý do báo cáo:</div>
                <div className="pl-4 p-2 bg-muted/30 rounded-md">{detail.Reason || detail.reason}</div>
              </div>
              {detail.Description && (
                <div>
                  <div className="font-semibold mb-2">Mô tả từ người dùng:</div>
              ...
              <div className="flex items-center justify-between p-3 bg-muted/20 rounded-lg">
                <div>
                  <div className="font-semibold mb-1 text-foreground">Trạng thái:</div>
                  <span className={cn(
                    "inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium",
                    detail.Status === "Resolved" ? "bg-emerald-100 text-emerald-700" :
                    detail.Status === "Ignored" ? "bg-amber-100 text-amber-700" :
                    "bg-red-100 text-red-700"
                  )}>
              ...
              {detail.historyReports && detail.historyReports.length > 0 && (
                <div className="p-3 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-200 dark:border-blue-800">
                  <div className="font-semibold mb-1 text-blue-700 dark:text-blue-300">Lịch sử báo cáo:</div>
                  <div className="text-sm text-blue-600 dark:text-blue-400">
                    Có <span className="font-bold">{detail.historyReports.length}</span> báo cáo khác cho cùng đối tượng này
                  </div>
                </div>
              )}
              ...
              <div className="pt-4 border-t border-border/20">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  {["Account", "BarPage", "BusinessAccount"].includes(detail.TargetType) && detail.Status !== "Resolved" && (
                    <button
                      type="button"
                      className={cn(
                        "px-4 py-2 rounded-lg text-sm font-medium transition-all",
                  ...
                  <button
                    ...
                    disabled={updatingId === (detail.ReportId || detail.reportId) || detail.Status === "Resolved"}
                  ...
                  <button
                    ...
                    disabled={updatingId === (detail.ReportId || detail.reportId) || detail.Status === "Ignored"}
                    onClick={() => handleAction(detail.ReportId || detail.reportId, "ignore")}
                  >
                    <XCircle size={16} />
                    Bỏ qua
                  </button>
      ...
      {viewingDetail && viewingDetail.type !== "post" && (
        <div className="fixed inset-0 z-[1300] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 overflow-y-auto" onClick={() => setViewingDetail(null)}>
          <div
            className="bg-card rounded-lg border border-border/20 w-full max-w-4xl p-6 my-8 shadow-xl max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-2">
